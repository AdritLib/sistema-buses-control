...
public interface UsuarioRepository extends JpaRepository<Usuario, Long>{
	...
	@Query(
			value = "SELECT NEW com.sistema_buses.dto.usuario.UsuarioResponse(u.id, u.nombre, u.correo, str(u.rol.nombre), u.activo) from Usuario u",
			countQuery = "SELECT count(u) FROM Usuario u"
			)
	Page<UsuarioResponse> listar(Pageable page);
	
	@Query(
			value = "SELECT NEW com.sistema_buses.dto.usuario.UsuarioResponse(u.id, u.nombre, u.correo, str(u.rol.nombre), u.activo) from Usuario u WHERE u.rol.nombre = 'CONDUCTOR'",
			countQuery = "SELECT count(u) FROM Usuario u"
			)
	Page<UsuarioResponse> listarConductores(Pageable page);
	
	@Query(
			value = "SELECT NEW com.sistema_buses.dto.usuario.UsuarioResponse(u.id, u.nombre, u.correo, str(u.rol.nombre), u.activo) from Usuario u WHERE u.rol.nombre = 'SUPERVISOR'",
			countQuery = "SELECT count(u) FROM Usuario u"
			)
	Page<UsuarioResponse> listarSupervisores(Pageable page);
